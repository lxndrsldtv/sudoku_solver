...
class SudokuSolverService {
  ...
  static Set<int> getPossibleValuesForCell({
    ...
    assert(cell.originValue == 0);

    final rowValues = sudoku.cells.where((c) => c.row == cell.row).map((c) => c.value);

    final columnValues = sudoku.cells.where((c) => c.column == cell.column).map((c) => c.value);

    final subgridValues = sudoku.cells.where((c) => c.subgrid == cell.subgrid).map((c) => c.value);

    return {1, 2, 3, 4, 5, 6, 7, 8, 9}
        .difference(rowValues.toSet().union(columnValues.toSet()).union(subgridValues.toSet()));
  }
  ...
  static SudokuModel fillCellsWithPossibleValues(SudokuModel sudoku) {
    return SudokuModel(
        cells: sudoku.cells
            .map((c) => c.copyWith(
                  possibleValues: c.value == 0 ? getPossibleValuesForCell(sudoku: sudoku, cell: c) : {},
                ))
            .toList());
  }

  static Set<SudokuCellModel> findSingletonCells(List<SudokuCellModel> subGridCells) {
    ...
    assert(subGridCells.where((c) => c.value == 0).every((c) => c.possibleValues.isNotEmpty));

    // join possibleValues of all cells to single list
    final possibleValuesOfAllEmptyCells =
        subGridCells.fold<List<int>>([], (p, c) => c.value == 0 ? p.followedBy(c.possibleValues).toList() : p);
    ...
    final subGridSingletonCells = subGridCells
        .where((cell) => cell.possibleValues.intersection(subGridSingletonValues).isNotEmpty)
        .map((c) => c.copyWith(value: c.possibleValues.intersection(subGridSingletonValues).first));
  ...
  static Stream<SudokuCellModel> process({required SudokuModel sudoku}) async* {
    ...
    var sudokuContainsEmptyCells = solvedSudoku.cells.firstWhereOrNull((c) => c.value == 0) != null;
    ...
    while (sudokuContainsEmptyCells) {
      ...
      final cellPossibleValues = getPossibleValuesForCell(sudoku: solvedSudoku, cell: cell);
      ...
      if (notAllPossibleValuesAreTested) {
        cell = cell.copyWith(
            value: untestedValues.min, possibleValues: {}, testedValues: cell.testedValues.union({untestedValues.min}));
      ...
      solvedSudoku.cells.replaceRange(cell.index, cell.index + 1, [cell]);
      yield (cell);
      sudokuContainsEmptyCells = solvedSudoku.cells.firstWhereOrNull((c) => c.value == 0) != null;
    }
  }

  static Stream<SudokuCellModel> fillSingletonCells({required SudokuModel sudoku}) async* {
