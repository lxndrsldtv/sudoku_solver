...
import 'package:sudoku_solver/services/sudoku_image_provider.dart';
...
class SudokuWidget extends StatelessWidget {
  ...
  SizedBox verticalDivider(
    ...
    final cellDivider = SizedBox(width: cellDividerSize, height: cellDividerSize);

    final subgridDivider = SizedBox(width: subgridDividerSize, height: subgridDividerSize);
  ...
  Row horizontalDivider(
      ...
      double cellDividerSize = 2.0,
      double subgridDividerSize = 4.0}) {
    final isSubgridBorder = (index + 1) % (sudoku.size.columns * sqrt(sudoku.size.columns)) == 0;

    final isSubgridLast = (index + 1) ~/ (sudoku.size.columns * sqrt(sudoku.size.columns)) == sqrt(sudoku.size.columns);

    const lastSubgridDivider = Row(children: [SizedBox(width: 0.0, height: 0.0)]);

    final ordinarySubgridDivider =
        Row(children: [SizedBox(width: cellSize * sudoku.size.columns, height: subgridDividerSize)]);

    final cellDivider = Row(children: [SizedBox(width: cellSize * sudoku.size.columns, height: cellDividerSize)]);
  ...
  List<Row> buildSudokuGrid(
    ...
    sudoku.cells.foldIndexed(<Widget>[], (index, prevValue, cell) {
      prevValue.add(Flexible(
          child: SudokuCellWidget(key: Key('ci${cell.index.toString()}'), cellSize: cellSize, cellIndex: cell.index)));
      // add vertical divider (SizedBox of some width between cells)
      prevValue.add(verticalDivider(
          index: index, sudoku: sudoku, cellDividerSize: cellDividerSize, subgridDividerSize: subgridDividerSize));
      ...
      if (rowFilledCompletely) {
        rows.add(Row(mainAxisAlignment: MainAxisAlignment.center, children: prevValue));
  ...
  Widget buildSizedBoxChild(
      SudokuBloc sudokuBloc, SettingsBloc settingsBloc, SudokuState state, double minScreenSideLength) {
    //--------------------------------------------------------------------------
    if (state is SudokuImageSelectionSucceed) {
      Future.delayed(const Duration(seconds: 1), () => sudokuBloc.add(SudokuImageRenderingDone()));
    ...
    if (state is SudokuCellRepositioning) {
      return TweenAnimationBuilder(
        duration: const Duration(seconds: 1),
        curve: Curves.easeInExpo,
        tween: Tween<double>(begin: 0.0, end: settingsBloc.state.settings.gridSettings.subgridDividerSize),
        onEnd: () {
        ...
        builder: (BuildContext context, Object? value, Widget? child) {
          return sudokuWidget(
            sudoku: sudokuBloc.state.sudokuModel,
            cellSideLength: cellSideLength(minScreenSideLength, settingsBloc),
            cellDividerSize: min((value as double), settingsBloc.state.settings.gridSettings.cellDividerSize),
    ...
    return sudokuWidget(
      ...
      subgridDividerSize: settingsBloc.state.settings.gridSettings.subgridDividerSize,
    );
  }
  ...
  double cellSideLength(double minOfScreenSides, SettingsBloc bloc) {
    ...
    final subgridDividerSize = bloc.state.settings.gridSettings.subgridDividerSize;
    final sudokuSize = bloc.state.settings.sudokuSettings.sudokuSize; // sudoku is square
    final sudokuSubgridSize = sqrt(sudokuSize); // subgrids are squares

    return (minOfScreenSides -
            (sudokuSize - 1 - (sudokuSubgridSize - 1)) * cellDividerSize.truncate() -
          ...
          child: BlocBuilder<PresentationBloc, PresentationState>(
            builder: (context, presentationState) {
              ...
              return Stack(alignment: Alignment.center, children: [
                Center(child: buildSizedBoxChild(sudokuBloc, settingsBloc, sudokuState, wh)),
                Visibility(
